...
$(document).ready(function(){
    $(".sub_button").click(function(){
        $.post('<?php echo osc_base_url(true); ?>', {email:$("#alert_email").val(), userid:$("#alert_userId").val(), alert:$("#alert").val(), page:"ajax", action:"alerts"},
            function(data){
                if(data==1) { alert('<?php echo osc_esc_js(__('You have sucessfully subscribed to the alert', 'classic')); ?>'); }
                else if(data==-1) { alert('<?php echo osc_esc_js(__('Invalid email address', 'classic')); ?>'); }
                else { alert('<?php echo osc_esc_js(__('There was a problem with the alert', 'classic')); ?>');
                };
        });
        return false;
    });
...
<div class="alert_form panel col-md-12">
<div class="row">
    <div class="tab-content">
    <h3>
        <strong><?php _e('Subscribe to this search', 'classic'); ?></strong>
    </h3>
    <form action="<?php echo osc_base_url(true); ?>" method="post" name="sub_alert" id="sub_alert" class="nocsrf">
            ...
            <button type="submit" class="sub_button btn btn-info btn-block"><?php _e('Subscribe now', 'classic'); ?>!</button>
            </div>
    </form>
    </div>
</div>
